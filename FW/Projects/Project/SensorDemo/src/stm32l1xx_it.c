...
* Return         : None
*******************************************************************************/
void SPI_IRQ_IRQHandler(void)
{    
#if ENABLE_MICRO_SLEEP
  Exit_LP_Mode();
#endif
    
  if(EXTI_GetFlagStatus(LIS_A_INT2_EXTI_LINE) == SET){
    
    if(GPIO_ReadInputDataBit(LIS_A_INT2_GPIO_PORT,LIS_A_INT2_PIN)){
        /* Interrupt from LIS331DLH */    
