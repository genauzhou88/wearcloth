...
* File Name          : SensorDemo_main_OTA.c
* Author             : AMS - AAS division
* Version            : V1.0.1
* Date               : 10-February-2014
* Description        : BlueNRG Sensor Demo main file with OTA support.
********************************************************************************
...
*******************************************************************************/
/**
 * @file  SensorDemo_main_OTA.c
 * @brief This application contains an example which shows how implementing a proprietary
 * Bluetooth Low Energy profile: the sensor profile.It also provides a reference example about
 * how using the Over-The-Air (OTA) bootloader capability. 
 *
...
* \subsection IAR_project_configurations IAR project configurations
  ...
  - \c LowPower_LowerApp_OTA - Low power configuration  with OTA service support (Start address 0x08003800)
  - \c LowPower_HigherApp_OTA - Low power configuration with OTA service support (Start address 0x0800E000)
  - \c USB_LowerApp_OTA - USB configuration  with OTA service support (Start address 0x08003800)
  - \c USB_HigherApp_OTA - USB configuration with OTA service support (Start address 0x0800E000)
  - \c OTA_Basic - Configuration to be used with OTA Service Manager (standalone BLE OTA bootloader). Start address 0x08006200.
...
@table
| LED name         | LowPower/USB      |*LowerApp_OTA  |*HigherApp_OTA  | OTA_Basic |
-------------------------------------------------------------------------------------
| D1               | Not used          |  Blinking     |  ON during OTA | Not used  |
| D2               | Blinking if error | ON during OTA |   Blinking     | Blinking  |           
| D3               | Not used          |  Not used     |   Not used     | Not used  |          
| D4               | Not used          |  Not used     |   Not used     | Not used  |      
| D5               | Not used          |  Not used     |   Not used     | Not used  |          
| D6               | Not used          |  Not used     |   Not used     | Not used  |         
@endtable
...
---------------------------------------------------------------------------------
| RESET                | Reset board                                            |       
| Push Button          | On RESET it allows to activate DFU / (2)               |                   
| Joystick Selection   | (1)                                                    |
@endtable

 - (1): On Reset, pressing this key the OTA EEPROM application base address information are cleaned
 ...
 * BlueNRG SensorDemo \see SensorDemo_main_OTA.c for documentation.
 *
 *@{
 */
...
int main(void)
{
    ...
    /* When ST_OTA_BTL is not enabled the 2 leds are used for debugging purposes */
    SdkEvalLedInit(LED1);
    SdkEvalLedInit(LED2);
...
#ifdef HIGHER_APP
    PRINTF("HIGHER APP RUNNING \r\n");
#else
    PRINTF("LOWER APP RUNNING \r\n");
#endif
#endif
